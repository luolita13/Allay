...
/// Rewrite Modrinth/CurseForge CDN URLs to MCIMirror.
/// Returns Some(rewritten_url) if the URL matched, None otherwise.
///
...
fn rewrite_mcimirror(url: &str) -> Option<String> {
    const REPLACEMENTS: &[(&str, &str)] = &[
        // Modrinth CDN (mod file downloads)
        ("https://cdn.modrinth.com/", "https://mod.mcimirror.top/"),
        // CurseForge CDN (mod / modpack file downloads)
...
mod tests {
    ...
    #[test]
    fn test_fetch_strategy_version_list_auto_official_first() {
        update_mirror_settings(1, 1, 1); // Auto for all
    ...
    fn test_fetch_strategy_auto_official_first() {
        ...
        update_mirror_settings(1, 1, 1); // Auto, Auto, Auto
    ...
    fn test_fetch_strategy_mirror_first() {
        update_mirror_settings(0, 0, 0); // Mirror, Mirror, Mirror
        match get_fetch_strategy("https://piston-data.mojang.com/v1/objects/abc/client.jar") {
    ...
    fn test_fetch_strategy_official_only() {
        update_mirror_settings(2, 2, 2); // Official, Official, Official
        match get_fetch_strategy("https://piston-data.mojang.com/v1/objects/abc/client.jar") {
            FetchStrategy::OfficialOnly => {}
    ...
    fn test_no_mirror_for_non_matching_urls() {
        update_mirror_settings(0, 0, 0); // Even with Mirror mode
    ...
    #[test]
    fn test_resolve_version_list_url() {
        update_mirror_settings(1, 1, 1); // Auto
